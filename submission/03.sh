#!/usr/bin/env bash
# What is the address of the P2SH script wrapping the witness redeem script in this transaction
# transaction=""

set -euo pipefail

BITCOIN_ARGS=()
if [[ -n "${BITCOIN_DATADIR:-}" ]]; then
  BITCOIN_ARGS+=(-datadir="$BITCOIN_DATADIR")
fi

bitcoin-cli() {
  if [[ ${#BITCOIN_ARGS[@]} -gt 0 ]]; then
    command bitcoin-cli "${BITCOIN_ARGS[@]}" "$@"
  else
    command bitcoin-cli "$@"
  fi
}

TRANSACTION=""

WITNESS_SCRIPT=$(bitcoin-cli -regtest decoderawtransaction "$TRANSACTION" | jq -r '.vin[0].scriptSig.asm | split(" ") | last')
WITNESS_SCRIPT_HASH=$(printf '%s' "$WITNESS_SCRIPT" | xxd -r -p | sha256sum | awk '{print $1}')
WITNESS_PROGRAM="0020$WITNESS_SCRIPT_HASH"

bitcoin-cli -regtest decodescript "$WITNESS_PROGRAM" | jq -r '.p2sh'
